...
- **Exact sample rate values** — frontend uses `EXACT_SAMPLE_RATES` lookup table (`39k: 39062, 78k: 78125, 156k: 156250, 312k: 312500`) matching server's `SAMPLE_RATES` dict in `sunsdr_direct.py`. Never use `parseInt(rateStr)*1000` approximation — the actual IQ rates are non-round values (e.g., 78125 not 78000).
