...
已切换到 **HTTPS(wss)** 解决。访问入口:`https://radio.vlsc.net:8080`。
...
### 根因
iOS Safari 只把 **HTTPS 源**当作安全上下文。手机用 `http://192.168.1.64:8080` 打开时:
...
### 验证(本机)
- 日志:`sunmrrc https://0.0.0.0:8080` + `TLS: .../fullchain.pem`;
  `Uvicorn running on https://0.0.0.0:8080`。
- `https://localhost:8080` → HTTP 200;用 `radio.vlsc.net` 域名校验 → HTTP 200(证书链完整)。
- 后端 SunSDR/IQ 流正常。

### 待你在 iPhone 上确认
- 用 `https://radio.vlsc.net:8080` 打开(域名要解析到这台机器)。
- 开机后应:出声 + 弹麦克风授权 + 控制可用。
...
- `./restart.sh`(默认 8080) / `WEB_PORT=8081 ./restart.sh` / `./restart.sh -f`(前台)
